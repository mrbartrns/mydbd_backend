...
class Owner(models.Model):
    killer = models.OneToOneField(
        Killer, on_delete=models.CASCADE, null=True, blank=True
    )
    survivor = models.OneToOneField(
        Survivor, on_delete=models.CASCADE, null=True, blank=True
    )
...
class Perk(models.Model):
    ...
    killer_or_survivor = models.CharField(
        max_length=256, choices=CATEGORY, default="Killer"
    )
    img_url = models.URLField(verbose_name="이미지 url", blank=True)
...
class ItemAddon(models.Model):
    ...
    name_kor = models.CharField(max_length=256, unique=True, verbose_name="한글 이름")
    item_category = models.ForeignKey(
        ItemCategory, on_delete=models.SET_NULL, null=True, related_name="addons"
    )
    description = models.TextField(verbose_name="설명")
    dt_created = models.DateTimeField(verbose_name="date created", auto_now_add=True)
    dt_modified = models.DateTimeField(verbose_name="date modified", auto_now=True)
...
class Category(models.Model):
    killer = models.OneToOneField(
        Killer, null=True, blank=True, on_delete=models.CASCADE
    )
    survivor = models.OneToOneField(
        Survivor, null=True, blank=True, on_delete=models.CASCADE
    )
    ...
    item_addon = models.OneToOneField(
        ItemAddon, null=True, blank=True, on_delete=models.CASCADE
    )
...
class Photo(models.Model):
    photo_category = models.ForeignKey(
        Category, on_delete=models.CASCADE, null=True, blank=True, related_name="photo"
    )
    image = models.ImageField(upload_to="images/apis", null=True, blank=True)
    dt_created = models.DateTimeField(verbose_name="date created", auto_now_add=True)
    dt_modified = models.DateTimeField(verbose_name="date modified", auto_now=True)
